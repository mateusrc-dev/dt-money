...
export const Content = styled(Dialog.Content)`
  // vamos estilizar o Dialog.Content
  min-width: 32rem;
  border-radius: 6px;
  padding: 2.5rem 3rem;
  ...
  form {
    ...
    button[type='submit'] {
      // vamos especificar qual o tipo do botão que vamos estilizar
      height: 58px;
      border: 0;
...
export const TransactionTypeButton = styled(
  ...
  &[data-state='unchecked']:hover {
    // vamos colocar no hover do botão não selecionado a seguinte estilização
    background: ${(props) => props.theme['gray-600']};
    transition: background 0.5s;
  }

  &[data-state='checked'] {
    //quando o botão estiver selecionado vai ter essa estilização
    color: ${(props) => props.theme.white};
    background: ${(props) =>
      props.variant === 'income'
